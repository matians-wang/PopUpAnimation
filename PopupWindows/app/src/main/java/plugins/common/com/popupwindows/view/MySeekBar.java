...
public class MySeekBar extends View implements
    ...
    float LINE_WIDTH = 3F;// 线宽 /** * 滑动指示器颜色 */
    ...
    public MySeekBar(Context context, AttributeSet attrs) {
        super(context, attrs); // 无视padding属性 使用内部定义的mPadding
        setPadding(0, 0, 0, 0);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) { // 获取自定义属性
        ...
        backLineColor = a.getColor(R.styleable.MySeekBar_back_line_color,
                Color.LTGRAY);
        indicatorLineColor = a.getColor(
                R.styleable.MySeekBar_indicator_line_color, Color.GREEN);
        indicatorColor = a.getColor(R.styleable.MySeekBar_indicator_color,
                Color.GRAY);
        a.recycle(); // 初始化SeekBar内部坐标对象
        ...
        mRightCI.setPoint(mPoints[mPoints.length - 1]); // 初始化Paint
        linePaint.setStyle(Paint.Style.STROKE);
        linePaint.setStrokeCap(Paint.Cap.ROUND);
        linePaint.setStrokeWidth(LINE_WIDTH); // 初始化滑动指示器Paint
        indicatorPaint.setStyle(Paint.Style.FILL);
        indicatorPaint.setColor(indicatorColor); // 设置阴影 注意：当前view需要添加
        // android:layerType="software"
        indicatorPaint.setShadowLayer(2, 2, 2, Color.LTGRAY); // 初始化圆形区域
        mRectF = new RectF();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        super.onSizeChanged(w, h, oldw, oldh); // CircleIndicator半径
        mR = h / 5 - mPadding; // 指定滑动指示器半径 mLeftCI.setR(mR);
        mRightCI.setR(mR); // 分段点坐标 mPoints数组 均分SeekBar宽度
        int y = h / 2;
        int perWidth = (w - 2 * mPadding - 2 * mR) / (mPoints.length - 1);
        System.out.println("perWidth=="+w);
        for (int i = 0; i < mPoints.length; i++) {
            mPoints[i].setX(mPadding + mR + i * perWidth);
            mPoints[i].setY(y);
        } // 更新一下 滑动指示器当前的坐标
        mLeftCI.setPoint(mLeftCI.getPoint());
        mRightCI.setPoint(mRightCI.getPoint()); // 回调当前Activity 告知2个滑动指示器的属性
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mPoints.length >= 2) {
            ...
            canvas.drawLine(mPoints[0].getX(), mPoints[0].getY(),
                    mPoints[mPoints.length - 1].getX(),
                    mPoints[mPoints.length - 1].getY(), linePaint);
            // 画区间线
            linePaint.setColor(indicatorLineColor);
            canvas.drawLine(mLeftCI.getCurX(), mLeftCI.getCurY(),
                    mRightCI.getCurX(), mRightCI.getCurY(), linePaint);
            // 画左边的Indicator
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: // 检查当前按下的坐标是否命中滑动指示器
                isSelected = checkPoint(x, y);
                break;
            case MotionEvent.ACTION_MOVE: // 在命中的情况下，滑动指示器会在有限范围内滑动
                move(x);
                break;
            case MotionEvent.ACTION_UP: // 当Up时，检查是否需要开启属性动画
                reset();
                break;
    ...
    private void reset() { // 重置滑动状态
        isSelected = false; // 执行动画
        if (mLeftCI.isTouch()) {
    ...
    private void statAnim(CircleIndicator rightCI, int closeIndex) {
        ObjectAnimator animator = ObjectAnimator.ofInt(rightCI, "curX",
                rightCI.getCurX(), mPoints[closeIndex].getX());
    ...
    private int getCloseIndex(CircleIndicator indicator) {
        ...
        if (indicator.equals(mLeftCI)) { // 如果是左边的Indicator,那么最大的index不能超过
            // 右边的Indicator所属的坐标index
            if (mPoints[index].getX() >= mRightCI.getCurX()) {
                index--;
            }
            return index;
        }
        if (indicator.equals(mRightCI)) { // 同理
    ...
    private void move(float x) {
        if (mLeftCI.isTouch()) { // 如果左边的Indicator呗拖拽，其x坐标应该在 第一个坐标
            // 和右边的Indicator 之间
            // 即限定 indicator可移动的范围
        ...
        if (mRightCI.isTouch()) {
            if (x <= mPoints[mPoints.length - 1].getX()
                    && x > mLeftCI.getCurX()) {
    ...
    /***
     * 检查 Down的x y是否命中 CircleIndicator，如果命中更新属性 @param x * @param y * @return
     * true 命中，false 为命中
     */
    private boolean checkPoint(float x, float y) {
        boolean containsL = mLeftCI.getRect().contains((int) x, (int) y);
        Rect rec = mLeftCI.getRect();
        System.out.println("mLeftCI.getRect==="+rec.left+"+"+rec.right+"+"+rec.top + "+"+rec.bottom);
        if (containsL) {
    ...
    /** * 设置2个Indicator的位置 * * @param left * @param right */
    ...
    public void onAnimationEnd(Animator animation) {
        CircleIndicator indicator = (CircleIndicator) ((ObjectAnimator) animation)
                .getTarget();
        // 更新被移动Indicator 坐标属性
        indicator.setPoint(mPoints[mCloseIndex]); // 动画执行结束
        isPlaying = false; // 回调Activity告知 滑动指示器信息
        callBack();
    }
    ...
    public class CircleIndicator {
        ...
        /** * 获取当前Indicator所在的矩形区域 * * @return Rect */
        public Rect getRect() {
